...
case "$(uname -s | tr '[:upper:]' '[:lower:]')" in
  msys*|mingw*|cygwin*|windows*) BOOTSTRAP_BIN="${BOOTSTRAP_BIN}.exe" ;;
esac
...
bootstrap_golangci_lint() {
  ...
  if [[ "$os" == "windows" ]]; then
    archive_url="https://github.com/golangci/golangci-lint/releases/download/${BOOTSTRAP_VERSION}/golangci-lint-${version_no_v}-${artifact}.zip"
    archive_path="${tmp_dir}/golangci-lint.zip"
  else
  ...
  local binary_name="golangci-lint"
  if [[ "$os" == "windows" ]]; then
