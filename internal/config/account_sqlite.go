...
func (s *accountSQLiteStore) ensureColumn(name, definition string) error {
	rows, err := s.db.Query(`PRAGMA table_info(accounts)`)
	if err != nil {
		return fmt.Errorf("inspect accounts sqlite schema: %w", err)
	}
	found := false
	for rows.Next() {
		...
		if err := rows.Scan(&cid, &columnName, &columnType, &notNull, &defaultValue, &primaryKey); err != nil {
			_ = rows.Close()
			return fmt.Errorf("scan accounts sqlite schema: %w", err)
		}
		if columnName == name {
			found = true
		}
	...
	if err := rows.Close(); err != nil {
		return fmt.Errorf("close accounts sqlite schema rows: %w", err)
	}
	if found {
		return nil
	}
	if _, err := s.db.Exec(`ALTER TABLE accounts ADD COLUMN ` + name + ` ` + definition); err != nil {
		return fmt.Errorf("add accounts sqlite column %s: %w", name, err)
	}
	return nil
}
...
func normalizeAccountConfig(acc Account) Account {
	...
	acc.DisabledReason = strings.TrimSpace(acc.DisabledReason)
	if !acc.Disabled {
		acc.DisabledReason = ""
