...
func NewFromEnv() *Store {
	...
	limit := parseIntWithDefault(os.Getenv("DEEPSEEK_WEB_TO_API_DEV_PACKET_CAPTURE_LIMIT"), defaultLimit)
	if limit < 1 {
		limit = defaultLimit
	}
	if limit > maxLimit {
	...
	maxBodyBytes := parseIntWithDefault(os.Getenv("DEEPSEEK_WEB_TO_API_DEV_PACKET_CAPTURE_MAX_BODY_BYTES"), defaultMaxBodyBytes)
