...
func TestCaptureRawSampleWritesPersistentSample(t *testing.T) {
	t.Setenv("DEEPSEEK_WEB_TO_API_RAW_STREAM_SAMPLE_ROOT", t.TempDir())
	...
	if got := rec.Header().Get("X-DeepSeek-Web-To-API-Sample-Id"); got != "my-sample-01" {
		t.Fatalf("expected sample id header my-sample-01, got %q", got)
	}
	if got := rec.Header().Get("X-DeepSeek-Web-To-API-Sample-Upstream"); got != filepath.Join(os.Getenv("DEEPSEEK_WEB_TO_API_RAW_STREAM_SAMPLE_ROOT"), "my-sample-01", "upstream.stream.sse") {
	...
	sampleDir := filepath.Join(os.Getenv("DEEPSEEK_WEB_TO_API_RAW_STREAM_SAMPLE_ROOT"), "my-sample-01")
...
func TestCaptureRawSampleCombinesContinuationCaptures(t *testing.T) {
	...
	sampleDir := filepath.Join(os.Getenv("DEEPSEEK_WEB_TO_API_RAW_STREAM_SAMPLE_ROOT"), "my-sample-02")
...
func TestCaptureRawSampleReturnsErrorWhenNoNewCaptureRecorded(t *testing.T) {
	root := t.TempDir()
	t.Setenv("DEEPSEEK_WEB_TO_API_RAW_STREAM_SAMPLE_ROOT", root)
	devcapture.Global().Clear()
	defer devcapture.Global().Clear()
