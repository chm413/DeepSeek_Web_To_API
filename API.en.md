...
`PUT /admin/accounts/{identifier}/proxy` accepts `{"proxy_id":"node-id","auto_route":false}` for a manual route or `{"auto_route":true}` for sticky automatic routing. `POST /admin/accounts/batch/actions` applies `set_proxy`, `enable`, or `disable` atomically to selected account identifiers; a route change clears stale tokens, syncs Xray, and reports bounded-concurrency relogin results. Automatic routing requires global `proxy_policy.auto_route_enabled=true` and a stored account password. The response includes the effective `proxy_id`, `auto_route`, `route_changed`, and `relogin`. Each item from `GET /admin/proxies` includes availability, latency, exit IP, country/colo, and assigned-account counts; its top-level `route_pool` is ordered by current assignment priority. Batch tests use `POST /admin/proxies/test-batch`, and the route policy is available through `GET`/`PUT /admin/proxies/policy`.
