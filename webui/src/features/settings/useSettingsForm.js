...
const DEFAULT_FORM = {
    ...
    prompt_limit: {
        ...
        session_chunking_enabled: false,
        session_chunking_target_ratio: 0.85,
        session_chunking_max_chunks: 16,
        session_chunking_commit_timeout_seconds: 30,
        summary_compaction_enabled: false,
        summary_compaction_threshold: 0.8,
...
function fromServerForm(data) {
    ...
    return {
        ...
        prompt_limit: {
            ...
            session_chunking_enabled: data.prompt_limit?.session_chunking_enabled ?? false,
            session_chunking_target_ratio: Number(data.prompt_limit?.session_chunking_target_ratio || 0.85),
            session_chunking_max_chunks: Number(data.prompt_limit?.session_chunking_max_chunks || 16),
            session_chunking_commit_timeout_seconds: Number(data.prompt_limit?.session_chunking_commit_timeout_seconds || 30),
            summary_compaction_enabled: data.prompt_limit?.summary_compaction_enabled ?? false,
            summary_compaction_threshold: Number(data.prompt_limit?.summary_compaction_threshold || 0.8),
        ...
        safety: {
            ...
            llm_check: {
                enabled: Boolean(data.safety?.llm_check?.enabled),
                model: data.safety?.llm_check?.model || 'deepseek-v4-flash-nothinking',
                timeout_ms: safeNumber(data.safety?.llm_check?.timeout_ms, 5000, 100, 60_000),
                fail_open: data.safety?.llm_check?.fail_open ?? true,
                cache_ttl_seconds: safeNumber(data.safety?.llm_check?.cache_ttl_seconds, 600, 1, 7 * 24 * 60 * 60),
                cache_max_entries: safeNumber(data.safety?.llm_check?.cache_max_entries, 10000, 1, 1_000_000),
                min_input_chars: safeNumber(data.safety?.llm_check?.min_input_chars, 30, 0, 1_000_000),
                max_input_chars: safeNumber(data.safety?.llm_check?.max_input_chars, 8000, 1, 1_000_000),
                max_concurrent: safeNumber(data.safety?.llm_check?.max_concurrent, 16, 1, 1024),
            },
        },
...
function toServerPayload(form) {
    ...
    return {
        ...
        prompt_limit: {
            ...
            session_chunking_enabled: Boolean(form.prompt_limit?.session_chunking_enabled ?? false),
            session_chunking_target_ratio: safeNumber(form.prompt_limit?.session_chunking_target_ratio, 0.85, 0.5, 0.95),
            session_chunking_max_chunks: safeNumber(form.prompt_limit?.session_chunking_max_chunks, 16, 2, 64),
            session_chunking_commit_timeout_seconds: safeNumber(form.prompt_limit?.session_chunking_commit_timeout_seconds, 30, 5, 300),
            summary_compaction_enabled: Boolean(form.prompt_limit?.summary_compaction_enabled ?? false),
            summary_compaction_threshold: safeNumber(form.prompt_limit?.summary_compaction_threshold, 0.8, 0.5, 0.95),
        ...
        safety: {
            ...
            llm_check: {
                enabled: Boolean(form.safety?.llm_check?.enabled),
                model: String(form.safety?.llm_check?.model || 'deepseek-v4-flash-nothinking').trim(),
                timeout_ms: safeNumber(form.safety?.llm_check?.timeout_ms, 5000, 100, 60_000),
                fail_open: Boolean(form.safety?.llm_check?.fail_open ?? true),
                cache_ttl_seconds: safeNumber(form.safety?.llm_check?.cache_ttl_seconds, 600, 1, 7 * 24 * 60 * 60),
                cache_max_entries: safeNumber(form.safety?.llm_check?.cache_max_entries, 10000, 1, 1_000_000),
                min_input_chars: safeNumber(form.safety?.llm_check?.min_input_chars, 30, 0, 1_000_000),
                max_input_chars: safeNumber(form.safety?.llm_check?.max_input_chars, 8000, 1, 1_000_000),
                max_concurrent: safeNumber(form.safety?.llm_check?.max_concurrent, 16, 1, 1024),
            },
        },
    }
}
