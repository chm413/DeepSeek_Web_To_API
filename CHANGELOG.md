...
优化账号分页和刷新：后台刷新、单账号操作和批量操作始终复用当前页、页大小和搜索条件，不再跳回第一页或显示第一条账号；过期的并发响应会被忽略，删除导致最后一页缩短时服务端会夹紧到最后有效页。账号选择可跨页保留，并可从页码输入框直接跳转。

## 2026-08-14 (1.1.3)

新增默认关闭的同上游会话原文分段协议：超限输入先由 `deepseek-v4-flash-nothinking` 规划并由服务端校验安全切点，随后在固定账号、固定会话和固定父消息链上逐段提交；每个内部请求重复强制输出格式，检测到消息 ID 与首个思考/正文片段后取消中间回答，并以随机探针和取消指令推进到下一段。该方案与既有 Flash 摘要压缩独立，启用时优先保留原文分段。同步增加详细上下文消耗日志、固定账号 PoW、Chat/Responses/Claude 与增量缓存命中支持，以及 WebUI 配置和校验。

新增默认关闭的粘性代理自动路由：最新检测成功的节点自动进入健康池，失败立即移出、恢复后重新加入；自动路由账号按已启用账号分配数最少优先，仅在首次分配或当前节点失效时迁移。节点切换会清除旧 Token、同步共享 Xray 并通过新出口重新登录；无健康节点时阻断自动路由账号，避免静默直连。管理台同步显示可用性、延迟、出口 IP、国家/地区、colo 和手动/自动分配数。
...
- 同会话原文分段完成真实超限验证：170,185 UTF-16 units 的用户输入（协议渲染后 174,293 units）超过 Pro 163,840 上限后被拆为 2 段；控制空流连续重试并确认父消息 ID 实际推进，最终 27.64 秒返回 HTTP 200 和严格 JSON `{"result":"CHUNK_OK"}`。
